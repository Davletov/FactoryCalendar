...
RowRenderer.prototype = {
	...
	rowHtml: function(rowType, row) {
		...
		// little hardcode !!!!!
		if(view.name == 'basicQuarter' || view.name == 'basicYear') {
			for (col = 0; col < view.colCnt; col++) {
				date = view.cellToDateForQuarter(row, col);
...
$.extend(DayGrid.prototype, {
	...
	// Converts a range with an inclusive `start` and an exclusive `end` into an array of segment objects
	rangeToSegs: function(start, end) {
		if(this.view.name == 'basicQuarter' || this.view.name == 'basicYear'){
			return this.view.rangeToSegmentsForQuarter(start, end); // for Quarters
		} else{
...
function View(calendar) {
	...
	t.getCellsPerQuarter = getCellsPerQuarter;
	t.getCellsPerYear = getCellsPerYear;
	...
	t.cellToDateForQuarter = cellToDateForQuarter;
	t.cellToCellOffset = cellToCellOffset;
	t.cellOffsetToDayOffset = cellOffsetToDayOffset;
	t.dayOffsetToDate = dayOffsetToDate;
	t.monthOffsetToDate = monthOffsetToDate;
	t.rangeToSegments = rangeToSegments;
	t.rangeToSegmentsForQuarter = rangeToSegmentsForQuarter;
	...
	function getCellsPerQuarter() {
		return 3;
	}

	function getCellsPerYear() {
		return 12;
	}
	...
	// - row, col
	// - { row:#, col: # }
	function cellToDateForQuarter() {
		var cellOffset = cellToCellOffset.apply(null, arguments);
		var dayOffset = cellOffsetToDayOffset(cellOffset);
		var date = monthOffsetToDate(dayOffset);
		return date;
	}
	...
	// month offset -> date
	function monthOffsetToDate(monthOffset) {
		return t.start.clone().add(monthOffset, 'months');
	}
	...
	// month -> day offset
	function monthToDayOffset(date) {
		var d1 = t.start._d;
		var d2 = date._d;
	...
	function dayOffsetToCellOffset(dayOffset) {
		...
		var result = Math.floor(dayOffset / 7) * cellsPerWeek + // # of cells from full weeks
	...
	// - isEnd
	//
	function rangeToSegmentsForQuarter(start, end) {
		...
		// day offset for given date range
		var dayRange = computeDayRange(start, end); // convert to a whole-day range
		var rangeDayOffsetStart = monthToDayOffset(dayRange.start);
		var rangeDayOffsetEnd = monthToDayOffset(dayRange.end); // an exclusive value

		// first and last cell offset for the given date range
		// "last" implies inclusivity
		var rangeCellOffsetFirst = dayOffsetToCellOffset(rangeDayOffsetStart);
		var rangeCellOffsetLast = dayOffsetToCellOffset(rangeDayOffsetEnd);
		...
		for (var row=0; row<rowCnt; row++) {
			...
			if (segmentCellOffsetFirst <= segmentCellOffsetLast) {
				...
				// range's first cell is hidden (we don't want isStart to be true).
				var isStart = cellOffsetToDayOffset(segmentCellOffsetFirst) == rangeDayOffsetStart;
				var isEnd = cellOffsetToDayOffset(segmentCellOffsetLast) == rangeDayOffsetEnd;
				                                                   // +1 for comparing exclusively
...
/* A year view with simple day cells running horizontally
----------------------------------------------------------------------------------------------------------------------*/
// TODO: a YearView mixin for calculating dates and titles

fcViews.basicYear = BasicYearView; // register this view

function BasicYearView(calendar) {
	BasicView.call(this, calendar); // call the super-constructor
}


BasicYearView.prototype = createObject(BasicView.prototype); // define the super-class
$.extend(BasicYearView.prototype, {

	name: 'basicYear',


	incrementDate: function(date, delta) {
		var result = date.clone().stripTime().add(delta, 'years').startOf('year');
		return result;
	},

	render: function(date) {

		this.intervalStart = date.clone().stripTime().startOf('year');
		this.intervalEnd = this.intervalStart.clone().add(1, 'years');
		...
		BasicView.prototype.render.call(this, 1, this.getCellsPerYear(), false); // call the super-method
	}

});
;;

/* A quarter view with simple day cells running horizontally
----------------------------------------------------------------------------------------------------------------------*/
// TODO: a QuarterView mixin for calculating dates and titles

fcViews.basicQuarter = BasicQuarterView; // register this view

function BasicQuarterView(calendar) {
	BasicView.call(this, calendar); // call the super-constructor
}


BasicQuarterView.prototype = createObject(BasicView.prototype); // define the super-class
$.extend(BasicQuarterView.prototype, {

	name: 'basicQuarter',


	incrementDate: function(date, delta) {
		var result = date.clone().stripTime().add(delta, 'quarters').startOf('quarter');
		return result;
	},
	...
	render: function(date) {
		...
		this.intervalStart = date.clone().stripTime().startOf('quarter');
		this.intervalEnd = this.intervalStart.clone().add(1, 'quarters');
		...
		BasicView.prototype.render.call(this, 1, this.getCellsPerQuarter(), false); // call the super-method
	}
...
$.extend(BasicWeekView.prototype, {
	...
	incrementDate: function(date, delta) {
		var result = date.clone().stripTime().add(delta, 'weeks').startOf('week');
		return result;
	},
